...
describe("handleChangesList", () => {
  it("calls execCli with changes list command", async () => {
    ...
    expect(mockExecCli).toHaveBeenCalledWith("changes", ["list"], {}, NETWORK_TIMEOUT);
...
describe("handleChangesApply", () => {
  it("calls execCli with id", async () => {
    ...
    expect(mockExecCli).toHaveBeenCalledWith(
      "changes", ["apply", "-i", "abc-123"],
      {}, NETWORK_TIMEOUT
    );
  });

  it("passes optional flags", async () => {
    await handleChangesApply({ id: "abc", snapshot: true, hash: "0x123", keepStaged: true });
    expect(mockExecCli).toHaveBeenCalledWith(
      "changes", ["apply", "-i", "abc", "--snapshot", "--hash", "0x123", "-k"],
      {}, NETWORK_TIMEOUT
    );
  });
});
...
describe("handleChangesReject", () => {
  ...
  it("passes optional flags", async () => {
    ...
    expect(mockExecCli).toHaveBeenCalledWith(
      "changes", ["reject", "-i", "abc", "--hash", "0x456", "-k"],
      {}, NETWORK_TIMEOUT
    );
  });
});
