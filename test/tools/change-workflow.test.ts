...
describe("change workflow args", () => {
  it("keeps staged assets only when a submitted change is applied immediately", () => {
    ...
    expect(
      buildChangeSubmissionArgs({ noApply: true, keepStaged: true })
    ).toEqual(["--no-apply"]);
  });

  it("adds immediate before submission flags", () => {
    expect(
      buildInstantChangeArgs({ immediate: true, keepStaged: true })
    ).toEqual(["-i", "-k"]);
  });
  ...
  it("builds change reject verification args", () => {
    expect(
      buildChangeRejectArgs({ id: "abc", hash: "0x456", keepStaged: true })
