...
export function pickJunoContext(
  ...
  if (capabilities.includes("mode") && mode) {
    context.mode = mode;
  }
  if (capabilities.includes("profile") && profile) {
    context.profile = profile;
  }
  if (capabilities.includes("containerUrl") && containerUrl) {
    context.containerUrl = containerUrl;
  }
  if (capabilities.includes("consoleUrl") && consoleUrl) {
    context.consoleUrl = consoleUrl;
  }
...
export function buildJunoContextArgs(
  ...
  const args: string[] = [];
  if (capabilities.includes("mode") && context?.mode) {
    args.push("--mode", context.mode);
  }
