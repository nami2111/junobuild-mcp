...
export const configInitSchema = z
  .object({
    format:
      ConfigFormatEnum.default("typescript").describe("Config file format"),
    source: z
      .string()
      .default("dist")
    ...
    satelliteId: z
      ...
      .describe(
        "Satellite ID for production environment. Use a real ID or leave the placeholder."
      ),
    multiEnv: z
      .boolean()
      .default(false)
      .describe(
        "Generate multi-environment config with staging and production satellite IDs"
      ),
    ...
    writeFile: z
      ...
      .default(false)
      .describe(
        "Write the config file directly to disk instead of returning content for preview"
      ),
    path: z
      .string()
      .optional()
...
export const configApplySchema = environmentFlagsBase
  .extend({
    force: z
      .boolean()
      .default(false)
      .describe("Overwrite configuration without checks"),
  })
  .strict();
